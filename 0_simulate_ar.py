...
import qsimpy
from arrivals import HeavyTailGamma, Autoregressive
...
def create_run_graph(params):
    ...
    service = Autoregressive(
        seed = params['service_seed'],
        level = 1,
        phi=[params['ar_correlation']],
        c = 0,
        dtype = 'float64',
        base_process=htg,
    )
    ...
    def user_fn(df):
        ...
        df['longer_delay_prob'] = np.float64(1.00) - service.base_process.cdf(
...
if __name__ == "__main__":
    ...
    project_path = str(p) + '/projects/ar_benchmark/'

    # simulation parameters
    bench_params = { # ar_correlation factor
        'l4':0.075, 
        'l3':0.025, 
        'l2':0.01, 
        'l1':0.005,
    ...
    for j in range(sequential_runs):
        ...
        for i in range(parallel_runs):
            ...
            params = {
                ...
                'ar_correlation' : bench_params[key_this_run], # tail decays
