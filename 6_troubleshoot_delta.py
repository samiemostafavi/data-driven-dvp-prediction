...
project_path = 'projects/p1_nolimit_benchmark/p1_results'
records_path = 'projects/p1_nolimit_benchmark/p1_results/queue_gym/'
...
for f in all_files:
    if f.endswith(".parquet"):
        file_addr = records_path + f
        df=spark.read.parquet(file_addr)
        q_len = df.first()['queue_length']
...
debug_json_addr = project_path + '/delta_debug.json'
...
for idx,item in enumerate(debug_dict):
    ...
    for dict_key in item.keys():
        ...
        for row_idx, row in dict_df.iterrows():
            ...
            cond_df = cond_df \
                .where( cond_df['queue_length'] == queue_length ) \
                .where( cond_df['longer_delay_prob'] == 1.00 )
