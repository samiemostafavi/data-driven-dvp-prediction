...
class FullGPDTailDistribution():
  ...
  def prob(self, values, norm_factor):
    ...
    #res = tf.multiply(norm_factor,tf.multiply(tf.divide(1,self.sigmas),tf.pow(1+tf.multiply(self.zais,tf.divide(self.values_sq-self.us,self.sigmas)),tf.divide(-self.zais-1.00,self.zais))))
    self.zai_is_0 = tf.multiply(norm_factor,tf.multiply(tf.divide(1,self.sigmas),tf.exp(tf.divide(self.values_sq-self.us,-self.sigmas))))
    self.zai_not_0 = tf.multiply(norm_factor,tf.multiply(tf.divide(1,self.sigmas),tf.pow(1+tf.multiply(self.zais,tf.divide(self.values_sq-self.us,self.sigmas)),tf.divide(-self.zais-1.00,self.zais))))
    self.max_x_u = tf.divide(-self.sigmas,self.zais)
    self.tres = tf.squeeze(tf.where(tf.equal(self.zais,0),self.zai_is_0,self.zai_not_0))
    self.tpres = tf.squeeze(tf.where(tf.less(self.values_sq - self.us,self.max_x_u),self.tres,tf.fill(tf.shape(self.tres),1e-15)))
    self.tppres = tf.where(tf.less(self.zais,0),self.tpres,self.tres)
    return tf.squeeze(self.tppres)
  ...
  def tail_prob(self, values, norm_factor):
    ...
    self.sigmas = tf.squeeze(self.tail_scale)
    # if zais are not zero
    #res = tf.multiply(norm_factor,tf.multiply(norm_factor,tf.pow(1+tf.multiply(self.cvalues_sq-self.cus,tf.divide(self.zais,self.sigmas)),tf.divide(-1,self.zais))))
    res = tf.where(
      tf.equal(self.zais,0),
      tf.multiply(norm_factor,tf.exp(tf.divide(self.cvalues_sq-self.cus,-self.sigmas))),
      tf.multiply(norm_factor,tf.multiply(norm_factor,tf.pow(1+tf.multiply(self.cvalues_sq-self.cus,tf.divide(self.zais,self.sigmas)),tf.divide(-1,self.zais)))))
...
class FullGPDExtremeValueMixtureDensityNetwork(BaseNNMixtureEstimator):
  ...
  def _build_model(self):
    ...
    with tf.variable_scope(self.name):
      ...
      self.layer_tail_param = L.NonlinearityLayer(slice_layer_tail_param, nonlinearity=None)
      self.tail_param = L.get_output(self.layer_tail_param)
      ...
      self.tail_dist = FullGPDTailDistribution(tail_threshold=self.tail_threshold,tail_param=self.tail_param,tail_scale=self.tail_scale)
    ...
    LayersPowered.__init__(self, [self.softplus_tail_scale, self.softplus_tail_threshold, self.layer_tail_param, self.softmax_layer_weights, self.softplus_layer_scales, self.softplus_layer_locs,
                                   self.layer_in_y])
  ...
  def _verbose(self, X, Y):
    ...
    print("tail res max_x_u: " + str(self.sess.run(self.tail_dist.max_x_u,feed_dict={self.X_ph: X, self.Y_ph: Y})))
    print("tail res zai_is_0: " + str(self.sess.run(self.tail_dist.zai_is_0,feed_dict={self.X_ph: X, self.Y_ph: Y})))
    print("tail res zai_not_0: " + str(self.sess.run(self.tail_dist.zai_not_0,feed_dict={self.X_ph: X, self.Y_ph: Y})))
    print("tail res:" + str(self.sess.run(self.tail_dist.tres,feed_dict={self.X_ph: X, self.Y_ph: Y})))
    print("tail p res:" + str(self.sess.run(self.tail_dist.tpres,feed_dict={self.X_ph: X, self.Y_ph: Y}))) 
    print("tail pp res:" + str(self.sess.run(self.tail_dist.tppres,feed_dict={self.X_ph: X, self.Y_ph: Y})))
