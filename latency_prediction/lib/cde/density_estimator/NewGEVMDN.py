...
class GPDTailDistribution():
  ...
  def tail_prob(self, values, norm_factor):
    ...
    self.sigmas = tf.squeeze(self.tail_scale)
    # if zais are not zero
    self.ctres = tf.multiply(norm_factor,tf.pow(1+tf.multiply(self.cvalues_sq-self.cus,tf.divide(self.zais,self.sigmas)),tf.divide(-1,self.zais)))
...
class NewGPDExtremeValueMixtureDensityNetwork(BaseNNMixtureEstimator):
