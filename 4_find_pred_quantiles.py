...
logger.info(F'Quantile range list: {qrange_list}')
...
for project_path in project_paths:
    ...
    for idx,pred_records_addr in enumerate(files):
        ...
        logger.info(F"Loaded predictor generated samples as a parquet file {pred_records_addr} with {df.count()} samples.")
        #df.show()
        #df.summary().show() #takes a very long time
        logger.info(f"Number of imported samples: {df.count()}")
        ...
        for index, row in conditions_table.iterrows():
            logger.info(f"-------- Calculating condition {index+1}/{len(conditions_table)}: {row.values}")
        ...
        results.to_csv(records_path + '/' +  Path(pred_records_addr).stem + ".csv")
